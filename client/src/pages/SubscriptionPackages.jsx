    ...
    <>
      <div className="relative">
        ...
        <div className="min-h-screen bg-[linear-gradient(to_right_bottom,rgba(0,0,0,0.6),rgba(0,0,0,0.6)),url('./src/assets/SubCover.jpg')] bg-center bg-cover">
          <h1 className="text-[#a80000] font-extrabold text-5xl text-center lg:text-5xl"><span className="text-white">Membership</span> Plans</h1>
          ...
          <div className="container mx-auto mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {subPackages.map((subPackage) => (
              <div key={subPackage._id} className="bg-white p-4 rounded-md shadow-md">
                <h3 className="text-lg font-bold">{subPackage.subPackageName}</h3>
                <p className="text-gray-500">${subPackage.price} / {subPackage.validTime} days</p>
                ...
                <div className="mt-4 flex justify-end">
                  <Link to={`/subpackages/details/${subPackage._id}`} className="text-blue-500 hover:underline">Details</Link>
                </div>
              </div>
            ))}
          </div>
