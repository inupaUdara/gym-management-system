...
const EditSubPackage = () => {
  ...
  useEffect(() => {
    setLoading(true);
    axios.get(`/api/subpackage/getIdSubPackage/${id}`)
    .then((response) => {
  ...
  const handleEditPackage = () => {
    ...
    axios
     ...
     .then(() => {
        setLoading(false);
        enqueueSnackbar("Package edit is uccessfully!", {variant: 'success'});
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex flex-col border-2 border-red-700 rounded-xl w-[600px] p-4 mx-auto">
        ...
        <button className="focus:outline-none font-bold text-white bg-red-700 hover:bg-red-800 rounded-lg text-sm px-5 py-3 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700" onClick={handleEditPackage}> 
          Save
        </button>
      </div>
    </div>
