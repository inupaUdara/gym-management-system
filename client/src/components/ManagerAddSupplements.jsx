...
export default function ManagerAddSupplements() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setLoading(true);
      setError(false);
      const res = await fetch('api/supplements/createSupplements', {
        method: 'POST',
        headers: {
      ...
      // navigate(`/admin-dashboard?tab=show-supplements/${data._id}`);
      ...
      <div className="max-w-[1000px] mx-auto rounded-md p-10 bg-white shadow-lg">
        <div className="flex flex-col mb-2">
          <main className='p-3 max-w-4xl'>
            <h1 className='text-3xl font-semibold text-center my-7'>Add Supplements</h1>
            <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
              <div className="flex flex-col gap-4 flex-1">
                <label htmlFor="productName" className="mt-3">Product Name :</label>
                <input type="text" placeholder='Enter product name' name="productName" className='border p-3 rounded-lg' id='productName' maxLength='50' minLength='1' required onChange={handleChange} value={formData.productName} />
                
                <label htmlFor="brandName" className="mt-3">Brand Name :</label>
                <input type="text" placeholder='Enter brand name' name="brandName" className='border p-3 rounded-lg' id='brandName' maxLength='50' minLength='1' required onChange={handleChange} value={formData.brandName} />

                <label htmlFor="category" className="mt-3">Category :</label>
                <select className='border p-3 rounded-lg' name="category" required onChange={(e) => setFormData({ ...formData, category: e.target.value})}>
              ...
              <div className="flex flex-col flex-1 gap-4">
                ...
                <button  disabled={loading || uploading} className='text-white text-sm my-2 bg-[#4c0000] border border-white rounded-md p-3 text-center flex items-center uppercase justify-center cursor-pointer hover:bg-[#7e1010]'> {loading ? 'Creating...' : 'Create listing'} </button>
