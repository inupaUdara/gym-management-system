...
export default function AdminDasInstructors() {
  ...
  const handleDeleteLeave = async () => {
    try {
        ...
        const data = await res.json();
        if (res.ok) {
            setLeaves((prev) => prev.filter((leave) => leave._id !== leaveIdToDelete));
            setShowModal(false);
        } else {
      ...
      {currentUser.isAdmin || currentUser.role === 'Manager'? (
        <>
          <Table hoverable className='shadow-md bg'>
            <Table.Head>
              <Table.HeadCell className='bg-[#1f1f1f] text-[#d4d4d4]'>Date of request</Table.HeadCell>
              <Table.HeadCell className='bg-[#1f1f1f] text-[#d4d4d4]'>Username</Table.HeadCell>
              <Table.HeadCell className='bg-[#1f1f1f] text-[#d4d4d4]'>Leave Type</Table.HeadCell>
              <Table.HeadCell className='bg-[#1f1f1f] text-[#d4d4d4]'>Start Date</Table.HeadCell>
              <Table.HeadCell className='bg-[#1f1f1f] text-[#d4d4d4]'>End date</Table.HeadCell>
              <Table.HeadCell className='bg-[#1f1f1f] text-[#d4d4d4]'>reason</Table.HeadCell>
              <Table.HeadCell className='bg-[#1f1f1f] text-[#d4d4d4]'>status</Table.HeadCell>
              <Table.HeadCell className='bg-[#1f1f1f] text-[#d4d4d4]'>status update</Table.HeadCell>
            ...
            {leaves.map((leave) => (
              <Table.Body className='divide-y' key={leave._id}>
                <Table.Row className='bg-[white] dark:border-gray-700 dark:bg-gray-800 text-[#1f1f1f]'>
                  ...
                  <Table.Cell className='text-green-400 cursor-pointer hover:underline'>Update</Table.Cell>
                  <Table.Cell>
                    <span
                      onClick={() => {
                        setShowModal(true);
