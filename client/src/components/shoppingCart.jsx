  ...
  return (  
    <div className="container mx-auto py-12">
      ...
      <div className="flex flex-col">
        ...
        {supplements.filter(supplement=>currentCartList.some(item=>item.id === supplement._id)).map(supplement=> {
          ...
          const cartItem = currentCartList.find(item=>item.id === supplement._id)
          const quantity = cartItem?.quantity
          const handleQuantityChange = (id,delta) => {
            const newItems = [...allQuantites.filter(item=>item.id !== supplement._id),{id, quantity: quantity + delta}]
            ...
            <div key={supplement._id} className="flex items-center mb-6">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex mt-2">
                  <span className="text-lg font-semibold">Rs.{sellingPrice}.00</span>
                  <button onClick={() => {
            const newItems = allQuantites.filter(item=>item.id !== supplement._id)
            localStorage.setItem('cart', JSON.stringify(newItems))
            setAllQuantites(newItems)
          }} className="text-red-500 font-medium ml-4 hover:underline">Remove</button>
                </div>
              </div>
              ...
              <div className="flex flex-col ml-auto">
                <form className="max-w-xs mx-auto">
                  ...
                  <div className="relative flex items-center max-w-[8rem]">
                    <button type="button" disabled={quantity<2} id="decrement-button" onClick={() => handleQuantityChange(supplement._id,-1)} className="bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 dark:border-gray-600 hover:bg-gray-200 border border-gray-300 rounded-s-lg p-3 h-11 focus:ring-gray-100 dark:focus:ring-gray-700 focus:ring-2 focus:outline-none">
                      <svg className="w-3 h-3 text-gray-900 dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 18 2">
                        <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M1 1h16"/>
                      </svg>
                    </button>
                    <input type="text" id="quantity-input" value={quantity} className="bg-gray-50 border-x-0 border-gray-300 h-11 text-center text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full py-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="999" required readOnly />
                    <button type="button" id="increment-button" onClick={() => handleQuantityChange(supplement._id,1)} className="bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 dark:border-gray-600 hover:bg-gray-200 border border-gray-300 rounded-e-lg p-3 h-11 focus:ring-gray-100 dark:focus:ring-gray-700 focus:ring-2 focus:outline-none">
